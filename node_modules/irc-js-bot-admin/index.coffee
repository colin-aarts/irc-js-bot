...
module.exports = ->
			...
			if '?' in input_data.flags
				...
				if 'a' in input_data.flags
					...
					if not (target_factoid of @factoids)
						...
						if /^alias:/.test @factoids[target_factoid]
							# target is an alias, so link to the original instead
							target_factoid = @factoids[target_factoid].replace /^alias:/, ''
				...
				# Regexp mode
					...
					if not factoid_exists
						...
						if not regexp_match
							...
							regexp_find = regexp_find.replace /// \\/ ///g, '\/'
							regexp_replace = regexp_replace.replace /// \\/ ///g, '\/'
	...
	@register_special_command
		...
		admin_only: true
		fn: (event, input_data, output_data) => throw new Error 'Force reload!'
	...
	@register_message_callback (event, input_data, output_data) =>
			...
			is_ignored_user = @user_match ignored_users, { nick: event.person.nick, host: event.person.host }
