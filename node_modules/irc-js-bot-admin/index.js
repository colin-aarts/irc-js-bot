...
(function() {
  ...
  module.exports = function() {
        ...
        if (__indexOf.call(input_data.flags, '?') >= 0) {
          ...
          if (is_alias) {
            ...
            if (!(__indexOf.call(input_data.flags, '!') >= 0)) {
              ...
              _this.save_factoids();
              message = "I successfully delete the factoid « " + factoid_name + " » and all aliases loading to it. The deleted factoid's content was « " + factoid_content + " »";
            }
          }
        }
        return _this.send('notice', event.person.nick, message);
    ...
    return this.register_message_callback(function(event, input_data, output_data) {
      var is_host_match, is_ignored_user, message, user, _i, _len;
      is_ignored_user = false;
      for (_i = 0, _len = ignored_users.length; _i < _len; _i++) {
        user = ignored_users[_i];
        is_host_match = user.host !== null ? (new RegExp("" + (js.re_escape(user.host)) + "$", 'i')).test(event.person.host) : false;
        if ((user.nick && user.nick === event.person.nick && !user.host) || (user.host && is_host_match && !admin.nick) || (user.host && is_host_match && user.nick && user.nick === event.person.nick)) {
