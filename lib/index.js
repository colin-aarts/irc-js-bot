...
(function() {
  ...
  bot = {
    ...
    connect: function() {
      ...
      (function(__iced_k) {
        ...
        return _this.irc.addListener('privmsg', function(event) {
          ...
          input_match = event.message.match(new RegExp("^(" + _this.re_triggers + ")(.+?)((\\s+([@>%])\\s+)(.*?))?((?:\\s+#\\s*)(.*))?$", 'i'));
    ...
    get_output_data: function(event, input_data) {
      var admin, is_host_match, output_data, _i, _len, _ref;
      output_data = {
        recipient: void 0,
        method: void 0,
        is_admin: false
      ...
      _ref = this.bot_options.admins;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        admin = _ref[_i];
        is_host_match = admin.host !== null ? (new RegExp("" + (js.re_escape(admin.host)) + "$", 'i')).test(event.person.host) : false;
        if ((admin.nick && admin.nick === event.person.nick && !admin.host) || (admin.host && is_host_match && !admin.nick) || (admin.host && is_host_match && admin.nick && admin.nick === event.person.nick)) {
    ...
    handle_command: function(event, input_data, output_data) {
      return this.special_commands[input_data.command].fn(event, input_data, output_data);
    },
